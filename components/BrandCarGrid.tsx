    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {cars.map((car) => (
        <Link key={car.id} href={`/cars/${car.slug || car.id}`}>
          <article className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 group cursor-pointer h-full">
            <div className="aspect-[4/3] bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden relative">
              {car.imageUrls[0] ? (
                ...
                <Image
                  src={BRAND_IMAGES[car.brand] ?? BRAND_IMAGES.default}
                  alt={car.brand}
                  fill
            ...
            <div className="p-5">
              ...
              <span className="inline-flex items-center text-sm font-medium text-[#0F172A] group-hover:text-[#DD5259] transition-colors">
                ดูรายละเอียด
